...
bool in_ea = false;
bool multiFirst = false;
bool multiSecond = false;
...
deque<state> exec_after, multi_op;
...
int isLockProtected(relax_element event1, relax_element event2)
{
  if (event1.tid == event2.tid)
	...
	for (std::deque<pair<ADDRINT,deque <pair<relax_element, relax_element>>>>::iterator lap = lock_addr_pair.begin();lap != lock_addr_pair.end(); ++lap)
	{
		for (std::deque <pair<relax_element, relax_element>>::iterator lp = lap->second.begin(); lp != lap->second.end(); ++lp)
		{
			...
			{
				if (lp->first.tid != up->first.tid) //(lp != up)
				{
					if((event1.tid == lp->first.tid && event2.tid == up->first.tid) && (event1.i_count <= lp->second.i_count && event2.i_count > up->first.i_count))
					{
						...
						{
							return lp->first.i_count;
						}
					}
					if((event2.tid == lp->first.tid && event1.tid == up->first.tid) && (event1.i_count <= up->second.i_count && event2.i_count > lp->first.i_count))
					{
						if (event1.i_count > up->first.i_count && event2.i_count <= lp->second.i_count)
						{
							return lp->first.i_count;
						}
					}
...
bool causallyPrecedes(relax_element event1, relax_element event2)
{
	...
	for (std::deque <pair<relax_element, relax_element>>::iterator fp = fork_pair.begin(); fp != fork_pair.end(); ++fp)
	{
		if (fp->first.tid == event1.tid && fp->second.tid == event2.tid) //first parent second child
		{
			...
			if (event1.i_count < fp->first.i_count) 
			{
				return true;
			}	
		}
		if (fp->first.tid == event2.tid && fp->second.tid ==event1.tid)//first child second parent 
		{
			...
			if (event2.i_count < fp->first.i_count) 
			{
				return true;
			}	
		}
	...
	for (std::deque <pair<relax_element, relax_element>>::iterator jp = join_pair.begin(); jp != join_pair.end(); ++jp)
	{
		if (jp->first.tid == event1.tid && jp->second.tid ==event2.tid) //first child
		{
			...
			if (event2.i_count >= jp->second.i_count)
			{
				return true;
			}
		}
		if (jp->first.tid == event2.tid && jp->second.tid ==event1.tid) // first parent
		{
			...
			if (event1.i_count >= jp->second.i_count)
			{
				return true;
			}	
		}
...
VOID ThreadFini(THREADID threadid, const CONTEXT *ctxt, INT32 code, VOID *v) {
	...
	std::cout << "+-------------------------------------------------------------------" << std::endl;*/
	    ...
	    cout << "PIN: Thread Finish Details:" << threadid  <<  endl;
	ThreadLocalData* tld = getTLS(threadid);
	

	
	if (threadid == tid2 && !done)
	{
		done = true;
	...
	for (std::deque<lockInfo>::iterator ad = lockAddr.begin(); ad != lockAddr.end(); ++ad)
	{
		...
		{
			if (threadid == adl->tid && adl->end == 0)
			{
				adl->end = tld->insCount;
			} 
		}		
	}
...
cout <<"check lock " << tid << " " << tld->insCount<<endl;
...
bool isLockUnlock(relax_element event)
{
	for (std::deque<lockInfo>::iterator ad = lockAddr.begin(); ad != lockAddr.end(); ++ad)
	{
		for (std::deque<lockedRegion>::iterator adl = ad->locked_region.begin(); adl != ad->locked_region.end(); ++adl)   
		{
			if (event.tid == adl->tid && (event.i_count == adl->start || event.i_count == adl->end))
			{
				return true;
			}	
		}		
...
bool isRaceSleep (THREADID t)
{
  if (threadFinished[t] == 1 || threadStarted[t] != 1)
  	return true;
  state st;
  st.tid = t;
	ThreadLocalData *tld = getTLS(t);
	st.count = tld->insCount;
	for (std::deque<sleep_element>::iterator it = race_sleep.begin(); it != race_sleep.end(); ++it)
	{
		if (it->tid == t && it->count == tld->insCount && !formerRelaxed(st))
		{
			return true;
		}
	}
...
VOID incrementThreadINS(THREADID tid, ADDRINT ins_addr, INS ins, CONTEXT *ctxt, int size, ADDRINT ip, bool last_bbl) {
	PIN_MutexLock(&mtx);
	totalins++;
	PIN_MutexUnlock(&mtx);
	ThreadLocalData *tld = getTLS(tid);
	...
	if (last_bbl)
	{
		if (active > 0)
		{
			if (semaphores[0].wait < 1)
			{
				//cout << "waiting final " << endl;
				semaphores[0].wait++;
				sem_wait(&semaphores[0].s);
			}
		}	
	}
  ...
  for (std::deque<startInfo>::iterator si_iter = startInfoMap.begin(); si_iter != startInfoMap.end(); ++si_iter)
  {
	...
	if (race && !first_run)
	{
		...
		if (!reached_breakpoint && !done)
		{
			...
			if ((tid == tid1) && (count1 == tld->insCount) && (!relax_same) && !multiFirst)
			{
				for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
				{
					if ((it->tid1 == tid1) && (it->count1 == count1) && (it->op1 == op1) && (it->type1 == type1) && (!it->executed1) )
					{
						passRelax = true;
						break;
					}
				...
				if (!passRelax && !done)
				{
					if (!executed && !formerRelaxed(curr_state))
					{
						...
						if (semaphores[tid].wait < 1 && !done)
						{
							//cout << "First state will wait " << tid << " " << order[tid].front().count << " " << order[tid][1].count<< endl;
							semaphores[tid].wait++;
							sem_wait(&semaphores[tid].s);
						}
					}
	...
	if (/*(!reached_breakpoint)&&*/(!done) && (!first_run))
	{
		...
		if (!pre_relax)
		{
			for (std::deque<state>::iterator it = exec_after.begin(); it != exec_after.end(); ++it)
			{
				...
				{
				  if ((!executed && curr_state.tid == tid) && (curr_state.tid != tid1 || curr_state.count != count1))
				  {
				  	break;
				  }
				  if ((executed && !waited && next_state.tid == tid) && (next_state.tid != tid1 || next_state.count != count1))
				  {
				  	break;
				  } 
					if (!reached_breakpoint)
					...
					else if (!in_ea)
					{
						...
						{
							bool relax_later;
							if ((!executed) && (curr_state.tid == tid1 || curr_state.count != count1 || curr_state.op != op1 || done))
							{ 
...
if (!first_run)
{
		...
		if (((tid == curr_state.tid && tld->insCount > curr_state.count) || (tid == next_state.tid && tld->insCount > next_state.count))  && order[tid].front().count != 0 && !done && !reached_breakpoint)
		{
			...
			if ((tid == next_state.tid) && (tld->insCount > next_state.count))
			{
				next_execute = true;
	                ...
	                {
	                  if (!(next_state.tid == tid1 && count1 == next_state.count && op1 == next_state.op) )
	                  {
			              	if (semaphores[next_state.tid].wait > 0)
			              	{
			              	  //out << "post next state: " << next_state.tid << endl;
			              		semaphores[next_state.tid].wait--;
			              		sem_post(&semaphores[next_state.tid].s);
			              	}
	...
	VOID MemoryReadInst(THREADID threadid, ADDRINT effective_address, ADDRINT read_addr, ADDRINT ins_addr, int i, bool preLock, UINT32 op_size)
	{
		...
		//if (lookup != memSet.end()) 
		{
			...
			if (multiFirst)
	  	{
				...
				{
					bool passRelax = false;
					if ((threadid == tid1) && (count1 == tld->insCount) && (op1 == i) && (type1 == 'r') && (!relax_same))
					{
						for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
						{
							if ((it->tid1 == tid1) && (it->count1 == count1) && (it->op1 == op1) && (it->type1 == type1) && (!it->executed1) )
				...
				if ((reached_breakpoint) && (!done) && (!relax_same))
				{
					if (threadid != tid2 || count2 != tld->insCount)
					{
						if (threadid == tid1 && count1 == tld->insCount)
						{
							if (hasStarted(tid2))
							{
								if (semaphores[tid2].wait > 0)
	   	...
	   	// PIN_LockClient();
			...
			PIN_MutexUnlock(&mtx);
			//PIN_UnlockClient();
			if ((threadid == break_point.tid) && (tld->insCount == break_point.count) /*&& (i == break_point.op) && ('r' == break_point.type)*/)
			{
				...
				else
				{
				  for (std::deque<relax_info>::iterator rf = relax_ds.begin(); rf != relax_ds.end(); ++rf)
        	{
        		if (rf->tid1 == tid1 && rf->op1 == op1 && rf->type1 == type1 && rf->count1 == count1)
        		{
        		  if (rf->executed2)
        		  {
			...
			if ((threadid == tid2) && (tld->insCount == count2) /*&& (i == op2) && ('r' == type2)*/)
			{
				////cout << "Done" << endl;
				second_done = true;
				done = true;
			...
			if (!first_run && !stack_end)
			{
				...
				if (((pre_executed) || (tld->insCount == curr_state.count)) && (curr_state.tid == threadid) && (!executed))
				{
					...
					if ((next_state.tid != tid1 || next_state.op != op1 || next_state.count != count1 || done) && !rel_wait)
					{
	      ...
	      if ((threadid == next_state.tid) && (tld->insCount == next_state.count) && (executed))
	        ...
	        //stack.pop_front();
					...
					if (semaphores[curr_state.tid].wait > 0 && !formerRelaxed(curr_state) && (done || curr_state.tid != tid1 || curr_state.count != count1) )
					{
	...
	VOID MemoryWriteInst(THREADID threadid, ADDRINT effective_address, ADDRINT * write_addr, ADDRINT ins_addr, int i, bool preLock, UINT32 op_size) {
		...
		for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
		{
			if ((it->tid1 == threadid) && (it->count1 == tld->insCount))
			{
				...
				write_relax.op1 = it->op1;
				write_relax.op2 = it->op2;
		...
		if (lookup != memSet.end()) {
			...
			for (std::deque<relax_element>::iterator se = racepoint_sleep.begin(); se != racepoint_sleep.end(); ++se)
			{
			  ...
			  if (formerRelaxed(sleepstate) && !laterExecuted(sleepstate) && !done)
				...
				if (se->tid == threadid && se->i_count == tld->insCount && !(se->tid == tid2 && se->i_count < count2 && reached_breakpoint && relax_second))	
				{
					...
					se->addr = effective_address;
					se->type = 'w';
					cout << "break 5 " <<i <<" " <<op1 <<" "<<type1 << " "<< count1 << " "<<tld->insCount <<" "<< reached_breakpoint<<done << endl; //error     
					if (!reached_breakpoint)
					{
						if (!executed && (curr_state.tid != tid1 || curr_state.count != count1))
						{
							if (semaphores[curr_state.tid].wait > 0)
							{
								//cout << "curr_state.tid post " << curr_state.tid << endl;
								semaphores[curr_state.tid].wait--;
								sem_post(&semaphores[curr_state.tid].s);
							}
							if (semaphores[threadid].wait < 1)
							{
								//cout << "tid wait " << threadid << endl;
								semaphores[threadid].wait++;
								sem_wait(&semaphores[threadid].s);
							}
						}
						if (!waited && executed && (next_state.tid != tid1 || next_state.count != count1))
						{
							if (semaphores[next_state.tid].wait > 0)
							{
								//cout << "curr_state.tid post " << next_state.tid << endl;
								semaphores[next_state.tid].wait--;
								sem_post(&semaphores[next_state.tid].s);
							}
							if (semaphores[threadid].wait < 1)
					...
					if (reached_breakpoint && !done && !(threadid == tid1 && tld->insCount == count1 && i == op1 && type1 == 'w'))
				  {
		 ...
		 if (multiFirst)
	  {
	  cout << "there it is y" << curr_state.tid << endl;
	  	if (!reached_breakpoint && !done)
	  	{
	  	cout << "there it is x" << curr_state.tid << endl;
	  		bool passRelax = false;
			if ((threadid == tid1) && (count1 == tld->insCount) && (op1 == i) && (type1 == 'w') && (!relax_same))
			{
				for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
				{
					if ((it->tid1 == tid1) && (it->count1 == count1) && (it->op1 == op1) && (it->type1 == type1) && (!it->executed1) )
				...
				if (!passRelax && !done)
				{
					if (!executed && !formerRelaxed(curr_state))
					{
						...
						if (semaphores[threadid].wait < 1 && !done)
						{
							//cout << "First state will wait " << threadid << " " << order[threadid].front().count << " " << order[threadid][1].count<< endl;
							semaphores[threadid].wait++;
							sem_wait(&semaphores[threadid].s);
						}
					}
	  	...
	  	if ((reached_breakpoint) && (!done) && (!relax_same))
		{
			...
			if (!skipRelax)
			{
				...
				if ((threadid == break_point.tid) && (tld->insCount == break_point.count) && (!reached_breakpoint))
				{
					...
					for (std::deque<relax_info>::iterator rf = relax_ds.begin(); rf != relax_ds.end(); ++rf)
        	{
        		if ((rf->tid1 == tid1 && rf->count1 == count1 && rf->op1 == op1  && rf->type1 == type1) && !(rf->tid2 == break_point.tid && rf->count2 == break_point.count))
        		{
        		  if (rf->executed2)
        		  {
        		    //cout << "post " << tid2 << endl;
			...
			if ((threadid == tid2) && (tld->insCount == count2) && (i == op2) && ('w' == type2))
			{
			...
			if (!first_run && !stack_end)
			{
				...
				if  (((pre_executed) || (tld->insCount == curr_state.count)) && (curr_state.tid == threadid) && (!executed))
				{
					...
					if ((next_state.tid != tid1 || next_state.count != count1 || next_state.op != op1 || done) && !rel_wait)
					{
						if (!(reached_breakpoint && !done && next_state.tid == tid1) )
						{
							if (semaphores[next_state.tid].wait > 0)
	...
	if (reached_breakpoint && !done)
	{
					...
					if ((wr->tid == it->tid1) && (wr->i_count1 == it->count1) && (!wr->executed1))
					{
						...
						if ((wr->tid == break_point.tid) && (wr->i_count1 == break_point.count) && (!reached_breakpoint))
						{
							...
							else
							{
								for (std::deque<relax_info>::iterator rf = relax_ds.begin(); rf != relax_ds.end(); ++rf)
								{
									if (rf->tid1 == tid1 && rf->count1 == count1 && rf->op1 == op1 && (rf->type1 == type1))
									{
										if (rf->executed2)
										{
											////cout << "post " << tid2 << endl;
		...
		if ((tid == tid2) && (tld->insCount == count2))
		{
			for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
			{
				if (it->tid1 == tid1 && it->count1 == count1 && it->op1 == op1 && it->type1 == type1 && !it->executed1 && it->executed2)
				{
					for (std::deque<writeRelax>::iterator wr = writeRelaxQueue.begin(); wr != writeRelaxQueue.end(); ++wr)
					{
						if (wr->tid == it->tid1 && wr->i_count1 == it->count1 && !wr->executed1)
						...
						if ((wr->tid == tid2) && (wr->i_count1 == count2) && (wr->op1 == op2) && ('w' == type2))
						{
								//cout << "Done" << endl;
								second_done = true;
								done = true;
		...
		void RecordReadAtBranch(THREADID tid, ADDRINT effective_address, ADDRINT ins_addr, int i, UINT32 op_size)
		{
		  ...
		  if ((tid == break_point.tid) && (tld->insCount == break_point.count))
			...
			if (reached_breakpoint && !done)
			{
			    ...
			    for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
			    {
			    	if (it->tid1 == tid2 && it->count1 == count2 /*&& it->op1 == op2*/)
			    	{
						    	...
						    	if (wr->tid == it->tid1 && wr->i_count1 == it->count1 && !wr->executed1)
							  	{
									...
									//if ((wr->tid == tid2) && (wr->i_count1 == count2) && (wr->op1 == op2) && ('w' == type2) && !done)
									{
									  	//cout << "Done" << endl;
									  	second_done = true;
									  	done = true;
			...
			if ((tid == tid2) && (tld->insCount == count2))
			{
				for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
				{
					if (it->tid1 == tid1 && it->count1 == count1 && it->op1 == op1  && it->type1 == type1 &&  !it->executed1 && it->executed2)
					{
						for (std::deque<writeRelax>::iterator wr = writeRelaxQueue.begin(); wr != writeRelaxQueue.end(); ++wr)
						{
							if (wr->tid == it->tid1 && wr->i_count1 == it->count1 && wr->op1 == it->op1 && !wr->executed1)
							{
								if ((wr->tid == break_point.tid) && (wr->i_count1 == break_point.count))
								{
									stack_end = true;
			...
			for (std::deque<writeRelax>::iterator wr = writeRelaxQueue.begin(); wr != writeRelaxQueue.end(); ++wr)
			{
				if (((wr->tid == curr_state.tid && wr->i_count1 == curr_state.count && !executed) || (wr->tid == next_state.tid && wr->i_count1 == next_state.count && executed && !waited)) && (!wr->executed1) && !done && !((wr->tid == tid1 && wr->i_count2 == relax_same_info.count2) /*&& (wr->i_count1 == count1)*/))
				{
					...
					for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
					{
						if (it->tid1 == wr->tid && it->count1 == wr->i_count1 && it->op1 == wr->op1)
						{
							it->executed1 = true;
			...
			if (tid == break_point.tid && tld->insCount == break_point.count)
		  {
			...
			if (reached_breakpoint && !done)
			{
				for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
				{
					if (tid == it->tid2 && tld->insCount == it->count2 && it->executed2)
					{
						if (it->tid1 != tid1 || it->count1 != count1 || it->op1 != op1)
						{
		  ...
		  PIN_LockClient();
			...
			for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
			{
				if (it->tid2 == tid && it->count2 == tld->insCount)
			  {
			    for (std::deque<writeRelax>::iterator wr = writeRelaxQueue.begin(); wr != writeRelaxQueue.end(); ++wr)
					{ 
					  if (wr->tid == it->tid1 && wr->op1 == it->op1 && wr->i_count1 == it->count1)
						{
						  it->executed2 = true;
					    wr->executed2 = true;
					    flushAll = false;
						    ...
						    if (wr->tid == it->tid1 && wr->i_count1 == it->count1  && wr->op1 == it->op1 && !wr->executed1)
						      ...
						      //cout << "Delete: and written new value: " <<*value_new<<" "<< &value_new << endl;
								  ...
								  second_done = true;
									...
									//PIN_UnlockClient();
									//cout << "pushing in bt: " <<wr->tid<<" "<< wr->i_count1 << endl;
									cout << "Details: " <<wr->tid<<" "<<tid2 <<" "<< wr->i_count1 <<" " <<count2<<" "<<wr->op1 <<" "<<op2<<" "<<type2<<done<< endl;
									//if ((wr->tid == tid2) && (wr->i_count1 == count2) && (wr->op1 == op2) && ('w' == type2) )
									{
									  //cout << "Done" << endl;
									  second_done = true;
...
//cout << "pushing in bt : " <<wr->tid <<" "<< wr->i_count1<<endl;
		  ...
		  ADDRINT value_wx;
			...
			for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
			{
				if (it->tid2 == tid && it->count2 == tld->insCount)
			  {
			    for (std::deque<writeRelax>::iterator wr = writeRelaxQueue.begin(); wr != writeRelaxQueue.end(); ++wr)
					{ 
					  if (wr->tid == it->tid1 && wr->i_count1 == it->count1 && wr->op1 == it->op1)
						{
						  it->executed2 = true;
					    wr->executed2 = true;
					    flushAll = false;
		...
		void 	rec_mem(INS ins, bool bbl_valid) {
			...
			/*if (INS_Disassemble(INS_Next(ins)).find("leave") != std::string::npos)
			{
				cout << "found leave" << tid << endl;
				if (tid == 0)
				{
					if (semaphores[tid].wait < 1)
					{
...
if (!first_run){
	...
	std::cout << "+-------------------------------------------------------------------" << std::endl;*/
		...
		VOID Pthread_join_callBefore(CHAR* name, pthread_t arg2, ADDRINT addr, pthread_t * pt, int i)
		{
			//PIN_GetLock(&l, 3);
			ThreadLocalData *tld = getTLS(PIN_ThreadId());
			//cout << "pthread join: "<<arg2 << " " << &arg2 << " " << pt[0] << " " << addr << " " << pt << " " <<&pt <<" "<<PIN_ThreadId()<< endl;
			...
			for (std::deque<threadInfo>::iterator ti_iter = threadInfoMap.begin(); ti_iter != threadInfoMap.end(); ++ti_iter)
			{
				...
				if ((ti_iter->regAddr == addr) && (ti_iter->end == 0))
				{
					...
					ti_iter->joined = true;
					for (std::deque<pair<relax_element, relax_element>>::iterator jp_iter = join_pair.begin(); jp_iter != join_pair.end(); ++jp_iter)
					{
						if (ti_iter->tid == jp_iter->first.tid && ti_iter->parent == jp_iter->second.tid && ti_iter->tid == ti_iter->tid)
						{
							jp_iter->second.i_count = tld->insCount;
							cout << "join pair updated " << jp_iter->second.tid<<" "<<jp_iter->first.tid<<" "<<jp_iter->second.i_count<<endl;
						}
					}
				}
	 ...
	 //RTN_InsertCall(pmlRtn, IPOINT_BEFORE, AFUNPTR(Pthread_join_callBefore), IARG_G_ARG0_CALLER, IARG_G_ARG1_CALLER,IARG_END);
			...
			if (RTN_Valid(pmlRtn) )
			{
				...
				RTN_InsertCall(pmlRtn, IPOINT_BEFORE, (AFUNPTR)thread_join_callBefore, 
					IARG_ADDRINT, "boost_thread_join", IARG_FUNCARG_ENTRYPOINT_VALUE, 
					0,IARG_THREAD_ID,
					IARG_CONTEXT, IARG_END);
				RTN_Close(pmlRtn);
			}
	    ...
	    // return;
			...
			ThreadLocalData *tld = getTLS(PIN_ThreadId());
			//PIN_ReleaseLock(&GlobalLock);
			bool next_bbl_valid = true;;
			//BBL last_bbl = TRACE_BblTail (trace);
			//INS last_ins = BBL_InsTail	(last_bbl);	
			//cout << "Last instruction " << PIN_ThreadId()<<" "<<INS_Disassemble(last_ins) << endl;
			...
			for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
			{
				...
				IMG img;
				RTN rtn;
				//next_bbl_valid = BBL_Valid(BBL_Next(bbl));
        		...
        		return;
					...
					if (RTN_Name(rtn).find("_fini") != std::string::npos)
						next_bbl_valid = false;
					if (INS_IsAtomicUpdate(ins)) {
						check_lock(ins);
					}
					rec_mem(ins, next_bbl_valid);
		...
		bool isRace(relax_element event1, relax_element event2)
		{
			if ((event1.type =='w' && event2.type =='w') && ((event1.value == event2.value))) 
			    return false;
			if (event1.tid != event2.tid && (event1.type =='w' || event2.type =='w') && event1.addr == event2.addr)
			{
			 	if (!causallyPrecedes(event1, event2) && isLockProtected(event1, event2) == -1)
					return true;
			}	
			else
				return false;
    ...
    /* For ech event check if it races or si reorderable with any previous event*/
			...
			for (std::deque<relax_info>::iterator rif = relax_ds.begin() ; rif != relax_ds.end(); ++rif)
		  {
	        ...
	        */
			if ((es->event.tid == tid2) && (es->event.i_count == count2))
			{
				...
				{
					if ((ss->tid == tid1) && ss->i_count == count1 && ss->op == op1 && ss->type == type1)
					{
						hasSleep = true;
						break;
					}
      ...
      /*Loop from  current event - 1 to the first event*/
					...
					if (es - bt_table.begin() >= race_point)
					{
                      ...
                      //break;
						...
						for (std::deque<relax_element>::iterator rs = racepoint_sleep.begin(); rs != racepoint_sleep.end(); ++rs)
						{
							...
							int lock_begin = isLockProtected(es->event, rs_relax);
							if (isRace(es->event, rs_relax)) 
							{
								rs->exhaust = true;
							}
							else if (lock_begin != -1 && !isLockAddr(es->event.addr))
							{
								if (!isLockUnlock(es->event))
								{
									bool started_later = false;
								  std::deque<bt_state>::iterator es_old = es;
									while (es_old - bt_table.begin() >= race_point)
									{
										es_old--;
										if (es_old->event.tid == es->event.tid && es_old->event.i_count == lock_begin)
										{
											started_later = true;
											break;
										}
        ...
        for (std::deque<relax_element>::iterator ss = es_pre->sleep_set.begin(); ss != es_pre->sleep_set.end(); ++ss)
				{
				  ...
				  { 
				  	cout <<"first check " << ss->tid <<" " << ss->i_count <<" "<<es->event.tid <<" "<<es->event.i_count << " "<< es_pre->event.tid <<" "<<es_pre->event.i_count <<endl;
				  	if (es->event.tid != es_pre->event.tid && !(ss->tid == es->event.tid && es->event.i_count == ss->i_count))
						{
						  if (isRace(*ss, es_pre->event) && isRace(*ss, es->event))
						  {
						    ...
						    { 
						      if ((in->tid == es->event.tid && es->event.i_count == in->i_count) || ((causallyPrecedes(es->event,*in) || isLockProtected(es->event,*in) != -1) && !isRelaxable(es->event,*in)) )
						      {
						        mid_race = false;
						        break;
						      }
				...
				if ((es->event.type == 'w' || es_pre->event.type == 'w') && (es->event.addr == es_pre->event.addr) && (es->event.tid != es_pre->event.tid) && (!(isLockAddr(es->event.addr))) && (!(isLockAddr(es_pre->event.addr))))
				{
					...
					cout << "checking " << es->event.tid <<" "<<es->event.i_count<<" "<< es_pre->event.tid <<" "<<es_pre->event.i_count<< endl;
					...
					cout << "checking 1" << endl;
					  ...
					  */
						if ((causallyPrecedes(*in, es->event) || isLockProtected(*in, es->event) != -1) && (!isRelaxable(*in, es->event)))
						{
							has_initial = true;
							for (std::deque<relax_element>::iterator re = es_pre->relax_event.begin(); re != es_pre->relax_event.end(); ++re)
							{
						  	if (!causallyPrecedes(*in,*re) && !isLockProtected(*in,*re) && in->addr != re->addr && in->tid != re->tid && (in->type == 'w' || re->type == 'w'))
						  	{
						    	has_initial = false;
						    	break;
						  	}
					...
					if (!causallyPrecedes(es->event, es_pre->event) && isLockProtected(es->event, es_pre->event) == -1)
					{
						if (!es_pre->initials.empty())
						{
							for (std::deque<relax_element>::iterator in = es_pre->initials.begin(); in != es_pre->initials.end(); ++in)
							{
								...
								cout << "checking previous initials 5"<< endl;	
						...
						if (!has_initial)
						{
							...
							else
							{
								...
								{
									if (initial->event.tid == es->event.tid)
									{	
										if (!causallyPrecedes(initial->event, es_pre->event) && isLockProtected(initial->event, es_pre->event) == -1)
										{
					...
					has_initial = false;
					if (!causallyPrecedes(es_pre->event, es->event) && isLockProtected(es_pre->event, es->event) != -1)
					{
						
						int lock_start_count = isLockProtected(es_pre->event, es->event);
						std::deque<bt_state>::iterator es_pre_lock = es_pre;
						for (; es_pre_lock >= bt_table.begin(); --es_pre_lock)
						{
							if (es_pre_lock->event.tid == es_pre->event.tid && es_pre_lock->event.i_count == lock_start_count)
								break;
						}
						if (es_pre_lock < bt_table.begin())
							goto later;
						for (std::deque<relax_element>::iterator ss = es_pre_lock->sleep_set.begin() ; ss != es_pre_lock->sleep_set.end(); ++ss)
						{
							state sleep_st;
							sleep_st.tid == ss->tid;
							sleep_st.count == ss->i_count;
							if (ss->tid == es->event.tid && ss->i_count <= es->event.i_count)
							{
								if (!formerRelaxed(sleep_st))
									has_initial = true;
							}
						
						}
						if (!es_pre_lock->initials.empty())
						{
							for (std::deque<relax_element>::iterator in = es_pre_lock->initials.begin(); in != es_pre_lock->initials.end(); ++in)
							{
								if ((in->tid == es->event.tid))
								{
									int temp_min,temp_max;
									...
									temp_min = in->i_count < es->event.i_count ? in->i_count : es->event.i_count;
									temp_max = in->i_count > es->event.i_count ? in->i_count : es->event.i_count;
									for (std::deque<relax_element>::iterator _in = es_pre_lock->initials.begin(); _in != es_pre_lock->initials.end(); ++_in)
									{
										if ((_in->tid == es->event.tid) && (_in->i_count == temp_min))
										{
											has_initial = true;
								...
								for (std::deque<bt_state>::iterator mid = es; mid > es_pre_lock; --mid)
								{
									if ((in->tid == mid->event.tid) && (in->i_count == mid->event.i_count))
									{
										if (!((in->type == 'w' || mid->event.type == 'w') && (in->addr == mid->event.addr) && (in->tid != mid->event.tid))) 
						...
						if (!has_initial)
						{
							if (es_pre_lock->initials.empty())
							{
								if (!causallyPrecedes(es->event, es_pre_lock->event)/* && isLockProtected(es->event, es_pre_lock->event) == -1*/)
								{
									es_pre_lock->initials.push_back(es->event);
								}	
							}
							else
							{
								for (std::deque<bt_state>::iterator initial = es_pre_lock + 1; initial <= es; ++initial)
								{
									if (initial->event.tid == es->event.tid)
									{	
										if (!causallyPrecedes(initial->event, es_pre_lock->event) /*&& isLockProtected(initial->event, es_pre_lock->event) == -1*/)
										{
											if (!((es_pre_lock->event.tid == initial->event.tid) && (es_pre_lock->event.i_count == initial->event.i_count)))
											{
												es_pre_lock->initials.push_back(initial->event);
											}
											total++;
											has_initial = false;
											break;
						...
						/*
						update the initials for locked regions
						*/
		...
		{
			bt_string = bt_string + std::to_string(es->event.tid) + "_" + std::to_string(es->event.i_count) + "_" + std::to_string(es->event.op) + "_" + es->event.type + "\n";
			bt_string = bt_string + "{";
			if ((rp == race_point) && (!((es->event.tid == tid2) && (es->event.i_count == count2))))
			{
				...
				{
					if (!causallyPrecedes(*re, es->event) && isLockProtected(*re, es->event) == -1)
					{
						bt_string = bt_string + std::to_string(re->tid) + "_" + std::to_string(re->i_count) + "_" + std::to_string(re->op)  + "_" + re->type + ",";
					}
				}	
			}
			else
			...
			for (std::deque<relax_element>::iterator ss = es->sleep_set.begin() ; ss != es->sleep_set.end(); ++ss)
			{
				bt_string = bt_string + std::to_string(ss->tid) + "_" + std::to_string(ss->i_count) + "_" + std::to_string(ss->op) + "_" + ss->type + ",";
			}
			if (bt_string.at(bt_string.length() - 1) == ',')
				bt_string = bt_string.substr(0, bt_string.length() - 1) + "}_<";
			else
			...
			if (((rp == race_point) && (es->event.tid == tid2) && (es->event.i_count == count2)) || (first_run) || (rp != race_point) || true)
			{
				for (std::deque<relax_element>::iterator rs = es->relax_event.begin() ; rs != es->relax_event.end(); ++rs)
				{
					if (bt_string.at(bt_string.length() - 1) != '<')
					{
						bt_string = bt_string.substr(0, bt_string.find_last_of("<")+1) + std::to_string(rs->tid) + "_" + std::to_string(rs->i_count)  + "_" + std::to_string(rs->op) + "_" + rs->type + "," + bt_string.substr(bt_string.find_last_of("<")+1);
		...
		//if (es->event.addr >= rs->addr)
			 // cout << "address relax violated " << rs->tid <<" "<<rs->i_count <<" "<< rs->addr <<" "<< es->event.tid <<" "<<es->event.i_count << " " << es->event.addr<< endl;
					...
					{
						bt_string = bt_string + std::to_string(rs->tid) + "_" + std::to_string(rs->i_count) + "_" + std::to_string(rs->op) + "_" + rs->type + ",";
							//if (es->event.addr >= rs->addr)
}
				}
			}
			...
			{
				bt_string = bt_string + std::to_string(ri->tid1) + "_" + std::to_string(ri->count1) + "_" + std::to_string(ri->op1) + "_"  + std::to_string(ri->tid2) + "_" + std::to_string(ri->count2) + "_" + std::to_string(ri->op2) + ",";
			...
			while (std::getline(file1, subs))
				{   
					...
					if (subs != "")
					{
						...
						subs = subs.substr(subs.find_first_of("_") + 1);
						st.op = std::stoi(subs.substr(0, subs.find_first_of("_")));
						st.index = stack.size();
	                ...
	                /*Reading Initials at state st in the previous run*/
							while (temp.find(",") != std::string::npos)
							{
								...
								t = t.substr(t.find_first_of("_") + 1);
								rx.op =  std::stoi(t.substr(0, t.find_first_of("_")));
	                ...
	                /*Reading Relaxable events at state st in the previous run*/
							...
							while (temp.find(",") != std::string::npos)
							{
								...
								t = t.substr(t.find_first_of("_") + 1);
								ri.op1 =  std::stoi(t.substr(0, t.find_first_of("_")));
								t = t.substr(t.find_first_of("_") + 1);
								ri.tid2 = std::stoi(t.substr(0, t.find_first_of("_")));
								t = t.substr(t.find_first_of("_") + 1);
								ri.count2 =  std::stoi(t.substr(0, t.find_first_of("_")));
								t = t.substr(t.find_first_of("_") + 1);
								ri.op2 =  std::stoi(t);
								ri.index = index;
	            ...
	            /*check if it has active relax*/
						while (!bt_prev[i].relax_event.empty())
						{
							...
							for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
							{
								if (it->tid1 == tid1 && it->count1 == count1 && it->op1 == op1 && it->type1 == type1 && it->type1 == type1)
								{
							...
							for (std::deque<relax_info>::iterator it = relaxed_ds.begin(); it != relaxed_ds.end(); ++it)
							{
								if ((it->tid1 == tid1) && (it->count1 == count1) && (it->op1 == op1) && (it->type1 == type1) && (it->count2 >= count1))
								{
									//it->count2 = it->count2 > count2 ? it->count2 : count2; 
									cout << "onw after  "<< it->tid1 << " " << it->count1 << " " << it->tid2 << it->count2 << endl;
									relax_done = true;
	            ...
	            /*checks if it has active race (not in sleep set)*/
					...
					for (int i = 0; i < race_point; i++)
					{
						if (!bt_prev[i].relaxed.empty())
						{
							for (int j = 0; j < bt_prev[i].relaxed.size(); j++)
							{
								...
								ri.op1 = bt_prev[i].relaxed[j].op1;
								ri.op2 = bt_prev[i].relaxed[j].op2;
	              ...
	              for (int l = race_point + 1; l < bt_prev.size(); l++) /*Check if the pair occurs after race_point but is recorded before*/
								...
								for (std::deque<relax_info>::iterator it = relaxed_ds.begin(); it != relaxed_ds.end(); ++it)
								{
									if ((it->tid1 == tid1) && (it->count1 == count1) && (it->op1 == op1)  && (it->type1 == type1) && (it->count2 >= count2))
									{
	  								...
	  								while (!bt_prev[i].initials.empty() && !new_race)
										{
											...
											if ((!in_sleep) && (bt_prev[i].event.tid != rx.tid))
											{
												...
												while (relax_ds.back().index >= i)
													relax_ds.pop_back();
												if (tid1 != ri.tid1 || count1 != ri.count1 || op1 != ri.op1)
													//relax_sub = false; // has race as well as relax 
												break;
											}
										}
...

{
	...
	for (std::deque<relax_element>::iterator it = racepoint_sleep.begin(); it != racepoint_sleep.end(); ++it)
	{
		se.tid = it->tid;
		se.count = it->i_count;
		if (!(se.tid == tid1 && se.count == count1 && se.op == op1 && se.type == type1) && !racepoint_relax )
		{
	...
	else
	{
		...
		break_point.op = bt_prev[race_point - 1].event.op;
		break_point.type = bt_prev[race_point - 1].event.type;
	}
	endrun.open("endrun.out");
	endrun << "race"  << endl;
	endrun.close();
...
//if (relax_sub)
{
	for (std::deque<relax_info>::iterator rs = relax_ds.begin(); rs != relax_ds.end(); ++rs)
	{
		 if (rs->tid1 == tid1 && rs->count1 == count1 && rs->op1 == op1 && rs->type1 == type1)
		 {
		   relax_same = true;
		   relax_same_info = *rs;
	...
	for (std::deque<state>::iterator it = multi_op.begin(); it != multi_op.end(); ++it)
	{
		if ((it->tid == tid1) && (it->count == count1))
		{
			multiFirst = true;
		}
		if ((it->tid == tid2) && (it->count == count2))
		{
			multiSecond = true;
		}
...
//relax_ds.clear();
cout << "RELAX and race at same point " << relax_same << " " << multiFirst <<" "<<type1 <<" "<<op1<< endl;;
